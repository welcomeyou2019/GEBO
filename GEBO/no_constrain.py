...
from lds_gnn.main_graph import get_main_graph

import far_ho as far
try:
    from lds_gnn.data import ConfigData, UCI, EdgeDelConfigData
    from lds_gnn.models import dense_gcn_2l_tensor_jk,GCN, gcn_train
    ...
    # noinspection PyUnresolvedReferences
    from models import dense_gcn_2l_tensor_jk, gcn_train
...
def lds(data_conf: ConfigData, config: LDSConfig, seed):
    # adj, labels, features, idx = get_main_graph()
    ...
    # idx_val = idx['val']
    # idx_test = idx['test']
    # train_mask = sample_mask(idx_train, adj.shape[0])
    # val_mask = sample_mask(idx_val, adj.shape[0])
    # test_mask = sample_mask(idx_test, adj.shape[0])
    ...
    with tf.Session(graph=g) as sess:
        ...
        out, ws = dense_gcn_2l_tensor_jk(placeholders, Z0, adj, g, num_layer=2, f_dim=f_dim, out_dim=out_dim)
        if data_conf.dataset_name == 'ppi':
            error = tf.identity(
        ...
        if config.train:
            try:
                for _ in es_gen:  # outer optimization loop
                    ...
                    # smp_1 = [sample(h) for h in far.utils.as_list(Z1)]
                    a = sess.run(smp_0)
                    # tf.math.reduce_sum(tf.where(a[0]<0))
                    # print('aaaaa', tf.math.reduce_sum(tf.where(a[0]<0)))
                    # print('bbbb', tf.math.reduce_sum(tf.where(a[0]>1)))
                    # print(np.sum(a[0],1))
                    # ss.run(smp_1)
                    ...
                    for j in range(config.io_params[2] // config.io_steps):  # inner optimization loop
                        ...
                        ho_step(steps(), tr_fd, val_all_fd, online=j)  # do one hypergradient optimization step
...
def main(data, method, seed, missing_percentage):
    ...
    if method == 'lds':
        configs = LDSConfig.grid(pat=20, seed=seed, io_steps=[5],
    ...
    for i in range(1):
        ...
        for cnf in configs:
            ...
            best_test_acc = 0
            print('cnf', cnf)
            valid_acc, test_acc, _, modify_adj = lds(data_config, cnf, seed)
            if best_valid_acc <= valid_acc:
                # print('Found a better configuration:', valid_acc)
                best_valid_acc = valid_acc
                best_test_acc = test_acc
                np.save('Z_jk' + data_config.dataset_name + str(cnf.io_steps)+'_'+str(i) +'_main.npy', modify_adj[0])

            print('Test accuracy of the best found model:', best_test_acc)
            result[cnf.io_steps].append(test_acc)
            print(result)
