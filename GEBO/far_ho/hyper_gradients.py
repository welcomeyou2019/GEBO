...
class HyperGradient(object):
    ...
    # far.optimizer.OptimizerDict
    @property
    def iteration(self):
        # print(self._optimizer_dicts)
        if self._iteration is None:
            # for opt_dict in sorted(self._optimizer_dicts):
            #     print('optim',opt_dict.iteration)
            self._iteration = [opt_dict.iteration for opt_dict in
                               sorted(self._optimizer_dicts)]  # far_ho.optimizer.OptimizerDict
...
class ReverseHG(HyperGradient):
    ...
    def compute_gradients(self, outer_objective, optimizer_dict, hyper_list=None):
        ...
        :return: list of hyperparameters involved in the computation
        """
        hyper_list = super(ReverseHG, self).compute_gradients(outer_objective, optimizer_dict, hyper_list)  # 获取超参
        # print(hyper_list)
        # print(list(optimizer_dict.state))
        # derivative of outer objective w.r.t. state
        # print(outer_objective.op.name)  error
        with tf.variable_scope(outer_objective.op.name):  # for some reason without this there is a cathastrofic
            # failure...
            # print(optimizer_dict.state)
            # print(optimizer_dict)
            doo_ds = tf.gradients(outer_objective, list(optimizer_dict.state))  # E对s的导数  alpha_T
            # print(doo_ds)

            alphas = self._create_lagrangian_multipliers(optimizer_dict, doo_ds)  # 论文中公式6中的alpha

            alpha_vec = utils.vectorize_all(alphas)  # 把所有梯度concat在一起？
            # print(alpha_vec)
            dyn_vec = utils.vectorize_all(list(optimizer_dict.dynamics))
            # print(dyn_vec)
            lag_phi_t = utils.dot(alpha_vec, dyn_vec,
                                  name='iter_wise_lagrangian_part1')  # outer和inner内积求均值，\sum alpha*phi
            # print(lag_phi_t)
            # TODO outer_objective might be a list... handle this case

            # iterative computation of hypergradients
            alpha_dot_B = tf.gradients(lag_phi_t, hyper_list)  # 公式10
            ...
            for dl_dh, a_d_b0, hyper in zip(alpha_dot_B, alpha_dot_B0, hyper_list):
                ...
                if dl_dh is not None:  # "normal hyperparameter"
                    ...
                    hyper_grad_step = tf.group(hyper_grad_step, hgv.assign_add(dl_dh))  # 更新g=g+alpha*B
            ...
            with tf.control_dependencies([hyper_grad_step]):  # first update hypergradinet then alphas.
                _alpha_iter = tf.group(*[alpha.assign(dl_ds) for alpha, dl_ds
                                         in zip(alphas,
                                                tf.gradients(lag_phi_t, list(optimizer_dict.state)))])  # alpht=alpha*A
    ...
    def run(self, T_or_generator, inner_objective_feed_dicts=None, outer_objective_feed_dicts=None,
        ...
        if not online:
            ...
            # print(self.initialization, initializer_feed_dict)
