...
def lds(data_conf: ConfigData, config: LDSConfig):
    ...
    #对于cora，citeseer之外的数据集
    # tvt_nids = pickle.load(open(f'data/graphs/{args.d}_tvt_nids.pkl', 'rb'))
    # train_mask = tvt_nids[0]
    # test_mask = tvt_nids[2]
    #
    # adj = pickle.load(open(f'data/graphs/{args.d}_adj.pkl', 'rb'))
    # features = pickle.load(open(f'data/graphs/{args.d}_features.pkl', 'rb'))
    # print(features[0])
    # ys = pickle.load(open(f'data/graphs/{args.d}_labels.pkl', 'rb'))
    ...
    # print(adj_hyp.eval())
    # adj_ini = constraint(preprocess_adj(adj))
    ...
    out, ws, rep = dense_gcn_2l_tensor(plc.X, adj, plc.Y, Z0, num_layer=config.num_layer,
                                       dropout=plc.keep_prob)
    ...
    if config.train:
        try:
            for _ in es_gen:  # outer optimization loop
                ...
                a_es = ss.run(acc, {**test_fd, out: result})
                e_es = ss.run(error, {**test_fd, out: result})
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', default='ppi', type=str,
